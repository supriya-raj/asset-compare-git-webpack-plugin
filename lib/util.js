...
const IGNORE_EXTENSIONS = /^(gz|map|jpe?g|png|gif|svg|woff2?|ico|ttf|eot|json|txt)$/i;
...
var _getExtension = function (str) {
  str = str.replace(/\?.*/, '');
  var split = str.split('.');
  var ext = split.pop();
  return ext
...
var _shouldIgnoreFile = function (str) {
  return IGNORE_EXTENSIONS.test(_getExtension(str));
};

var getAssets = (stats) => {
  return stats.assets.reduce((assets, asset) => {
    let chunkName = asset.chunkNames.join(' ') || asset.name;

    if (!_shouldIgnoreFile(asset.name)) {
      if (_.isArray(stats.assetsByChunkName[chunkName]) && stats.assetsByChunkName[chunkName].filter((file) => !_shouldIgnoreFile(file)).length > 1) {
        chunkName = chunkName + '.' + _getExtension(asset.name)
      }
      assets.push({
        name: chunkName,
        size: asset.size
